...
@testset "Analytical Solutions" begin
    @testset "GBM analytical functions" begin
        ...
        @test gbm_variance(μ, σ, u0, 0.0) ≈ 0.0 atol = 1e-15
        ...
        monte_prob = EnsembleProblem(prob)
        sol = solve(monte_prob, EM(); dt = 0.001, trajectories = 100000)
        ...
        @test abs(simulated_mean - analytical_mean) / analytical_mean < 0.02
        @test abs(simulated_var - analytical_var) / analytical_var < 0.05
    end
    ...
    @testset "OU analytical functions" begin
        ...
        @test ou_variance(a, σ, 0.0) ≈ 0.0 atol = 1e-15
        ...
        large_t = 100.0
        @test ou_mean(a, r, u0, large_t) ≈ ou_stationary_mean(r) atol = 1e-10
        @test ou_variance(a, σ, large_t) ≈ ou_stationary_variance(a, σ) atol = 1e-10
        ...
        @test abs(simulated_mean - analytical_mean) < 0.02
        @test abs(simulated_var - analytical_var) / analytical_var < 0.05
    end
    ...
    @testset "CIR analytical functions" begin
        ...
        @test cir_variance(κ, θ, σ, u0, 0.0) ≈ 0.0 atol = 1e-15
        ...
        # Test convergence to stationary distribution
        large_t = 100.0
        @test cir_mean(κ, θ, u0, large_t) ≈ cir_stationary_mean(θ) atol = 1e-10

        # Monte Carlo validation
        prob = CIRProblem(κ, θ, σ, u0, (0.0, t))
        monte_prob = EnsembleProblem(prob)
        sol = solve(monte_prob, EM(); dt = 0.0001, trajectories = 100000)
        final_values = [sol[i].u[end] for i in eachindex(sol)]
        simulated_mean = mean(final_values)
        simulated_var = var(final_values)
        ...
        @test abs(simulated_mean - analytical_mean) / analytical_mean < 0.02
        @test abs(simulated_var - analytical_var) / analytical_var < 0.1
    end
    ...
    @testset "Black-Scholes log-price functions" begin
        ...
        @test bs_log_variance(σ, 0.0) ≈ 0.0 atol = 1e-15
    ...
    @testset "Heston analytical functions" begin
        ...
        @test heston_variance_variance(κ, Θ, σ, u0_v, t) ≈ cir_variance(κ, Θ, σ, u0_v, t)
    end
end # testset Analytical Solutions
